...
## Previously learned equations

0. membrane potential caused by stochastic arriving spikes and external current in SRM0 model

	$$
	\begin{equation}
	h_\mathrm{PSP}(t) = J_0 \int_0^{\infty} \eta_0(s)A(t-s)ds + \int_0^{\infty}\xi_0(s)I^\mathrm{ext}(t-s)ds
	\end{equation}
	$$
